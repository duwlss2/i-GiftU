...
<main>
  ...
  <div style="display:inline;">
   ...
   <?php
       ...
       $page_scale = 10;
       $temp = floor(($page_n) / $page_scale) * $page_scale ;

       //여기에서도 restaurant먼저 분류 필요
       $sql = "SELECT * from purchase_history order by coupon_no DESC limit $content, $rec_num";
       $result = mysqli_query($conn, $sql);
       while($row = mysqli_fetch_assoc($result))
       {
         $sql = "SELECT * from ".$row['restaurant']." WHERE menu = '".$row['menu']."'";
         ...
         $row4 = mysqli_fetch_assoc($result4);
          ...
          <table class="bordered" <?php echo $used; ?>>
            <tr>
              ...
              <td width = 130px style="padding-left:20px"><?php echo $row4['name'] ?></td>
              <td width = 100px style="padding-left:30px"><b><?php echo $row['deposit_name'] ?></b></td>
              <td width = 60px style="padding-left:10px"><b><?php echo $row4['price'] ?></b></td>
      ...
      <td width = 110px>
          ...
          $(document).ready(function(){
            $('.<?php echo 'labelchk'.$coupon_no; ?>').on("click", function(e){
              ...
              $.ajax({
                ...
                data: {c_no: <?php echo $coupon_no;?>, is_deposit: <?php echo $is_deposit; ?>, is_used: <?php echo $is_used; ?>},
                success: function(result) {
                  if(result === '사용'){
                    alert("이미 사용된 쿠폰입니다.");
                    $('.<?php echo 'chk'.$coupon_no; ?>').prop('checked', true);
                  } else if(result === '입금확인'){
                    $('.<?php echo 'chk'.$coupon_no; ?>').prop('checked', true);
                    alert("입금처리 되었습니다.");
                  } else if(result === '입금취소') {
                    $('.<?php echo 'chk'.$coupon_no; ?>').prop('checked', false);
                    alert("입금취소 처리되었습니다.");
                  }
                  location.reload();
                }
        ...
        <ul class="pagination">
         <?php
         if( $temp > 1 ){
           echo "<li><a href = $_SERVER[PHP_SELF]?start=0>끝</a></li>";
         echo "<li><a href = $_SERVER[PHP_SELF]?start=$front>다음</a></li>";
         }
        echo '<li class="disabled"><i class="material-icons">chevron_left</i></li>';

        for($i=0;$i<=$page_scale;$i++){
          $print = $temp+$i;
          if($print > $page_cnt) break;
          if($print == $page_n){
            $page = "<b>".$print."</b>";
          } else {
            $page = $print;
          }
          echo "<li><a href = $_SERVER[PHP_SELF]?start=$print>$page</a></li>";
        }
        echo '<li class="disabled"><i class="material-icons">chevron_right</i></li>';

        if( $temp + $page_scale <= $page_cnt ){
        echo "<li><a href = $_SERVER[PHP_SELF]?start=$back>이전</a></li>";
        echo "<li><a href = $_SERVER[PHP_SELF]?start=$page_cnt>처음</a></li>";
        }
        ?>
          </ul>
...
<footer>
  ...
  <b>CRA</b><br>
  문제시 연락처 : 010.8222.3574 010.8523.1494 로 연락주세요
  </p>
</footer>
